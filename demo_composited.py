...
while(True):
    ...
    prediction_pil = np.array(colorized.convert('RGB'))
    prediction_pil = cv2.cvtColor(np.array(prediction_pil), cv2.COLOR_RGB2BGR)

    composited = cv2.addWeighted(frame, 1, prediction_pil, 0.5, 0)

    end = time.time()
    print("執行時間：%f 秒" % (end - start))
    cv2.imshow('Realtime Segmentation', composited)
